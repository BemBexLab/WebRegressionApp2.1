...
function storagePathFromUrl(url: string): string | null {
  ...
  try {
    ...
    if (parsed.port === "5000" || parsed.hostname === "supabase-storage") {
...
function resolveAssetUrl(url: string | null): string | null {
  ...
  const storageOrigin = trimTrailingSlash(
    process.env.SUPABASE_STORAGE_URL ?? "http://supabase-storage:5000"
  );
  const storagePath = storagePathFromUrl(url);
  if (storagePath) {
    return `${storageOrigin}${storagePath.startsWith("/") ? storagePath : `/${storagePath}`}`;
  }
...
function availableImageHeight(doc: PDFKit.PDFDocument): number {
  return doc.page.height - doc.y - doc.page.margins.bottom;
}

async function drawImageAcrossPages(
  ...
  const scale = printableWidth / metadata.width;
  let top = 0;

  while (top < metadata.height) {
    let sliceHeightPx = Math.floor(availableImageHeight(doc) / scale);

    if (sliceHeightPx <= 0) {
      doc.addPage();
      sliceHeightPx = Math.floor(availableImageHeight(doc) / scale);
    }

    sliceHeightPx = Math.max(1, Math.min(sliceHeightPx, metadata.height - top));
    ...
    doc.image(sliceBuffer, undefined, undefined, {
      width: printableWidth,
    });

    top += sliceHeightPx;
    if (top < metadata.height) {
      doc.addPage();
    }
  }
}
...
async function addPageSection(
  ...
  for (const image of images) {
    ...
    if (image.buffer) {
      doc.moveDown(0.3);
      await drawImageAcrossPages(doc, image.buffer);
